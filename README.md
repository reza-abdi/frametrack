...
The seed optionally creates or updates the admin configured by
`ADMIN_EMAIL`, `ADMIN_PASSWORD`, and `ADMIN_NAME`; it does not create a
hard-coded account. It also adds sample inventory idempotently.
...
deployment workflow before releasing code that depends on a new migration. If
an existing database was previously managed with `db push`, baseline it before
the first deployment instead of applying the initial migration over its tables.
...
4. Run `npm run db:migrate:deploy` through a protected production workflow,
   then deploy the application build. Never run `prisma db push` in Vercel
   builds.
