...
export default function PhonePairPage({ params }: PageProps) {
  ...
  const applyStatus = useCallback((res: Response, data: { error?: string }) => {
    if (res.status === 404) {
      setSession({
        ...
        message:
          data.error ??
          "Pairing ended — the session was closed on the computer.",
      });
      setScannerOn(false);
      return;
    }
  ...
  const handleScanned = useCallback(
    async (barcode: string) => {
      ...
      setSending(true);
      setError(null);
      const res = await fetch(`/api/pair/${encodeURIComponent(code)}/scan`, {
        method: "POST",
      ...
      <div className="mx-auto mt-16 max-w-md">
        <div className="card p-6 text-center">
          <h1 className="text-xl font-semibold text-slate-900">
            {session.status === "ended" ? "Pairing ended" : "Pair code invalid"}
          </h1>
